...
export function handleClickExtraer () {
    ...
    if (resultado && resultado.infoIban) {
        document.getElementById('formado')!.innerText = `¿Está bien formado?: ${resultado.iban ? 'Sí' : 'No'}`;
        document.getElementById('valido')!.innerText = `¿Es válido?: ${resultado.iban ? 'Sí' : 'No'}`;
        document.getElementById('banco')!.innerText = `Banco: ${resultado.banco}`;
        document.getElementById('sucursal')!.innerText = `Sucursal: ${resultado.infoIban.codSucursal}`;
        document.getElementById('digito-control')!.innerText = `Dígito de control: ${resultado.infoIban.digitoCtrlB}`;
        document.getElementById('numero-cuenta')!.innerText = `Número de cuenta: ${resultado.infoIban.cuenta}`;
    } else {
